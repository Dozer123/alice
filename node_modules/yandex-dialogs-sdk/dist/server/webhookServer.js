...
const isAvailableMethod = (method) => {
    if (method === undefined) {
        return false;
    }
    return ['POST', 'OPTIONS'].includes(method);
};
const formatWebhookUrl = (webhookUrl) => {
    if (webhookUrl === '') {
        return '/';
    }
    return webhookUrl;
...
class WebhookServer {
    constructor(config) {
        ...
        this.server = http.createServer(config.options, (request, response) => this._handleRequest(request, response));
    }
    ...
    start() {
        ...
        this.server.listen(this.port, () => {
            debug_1.default(`server is listening on ${this.port}, '${this.webhookUrl}'`);
